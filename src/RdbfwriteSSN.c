...
#include "foreignSSN.h"



static DBFHandle RdbfwriteSSN(DBFHandle, SEXP, SEXP, SEXP, SEXP);
...
SEXP DoWritedbfSSN(SEXP file, SEXP df, SEXP pr, SEXP sc, SEXP DataTypes)
{
    ...
    RdbfwriteSSN(hDBF, df, pr, sc, DataTypes);
    DBFClose(hDBF);
    return R_NilValue;
}
...
static DBFHandle
RdbfwriteSSN(DBFHandle hDBF, SEXP df, SEXP pr, SEXP sc, SEXP DataTypes)
{
	...
	for(i = 0; i < nflds; i++) {
		...
		if(ISNAN(rtmp))
		    ...
		    DBFWriteDoubleAttributeSSN(hDBF, iRecord, i, rtmp);
		break;
	    case STRSXP:
